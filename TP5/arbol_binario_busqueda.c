...
NodoArbol abb_insertar_recursivo(ArbolBinarioBusqueda a, NodoArbol pa, TipoElemento te) {
    if (pa == NULL) {
        ...
        if (a->cantidad_elementos == 1) a->raiz = nuevo_nodo;
    
        return nuevo_nodo;
    } 
    
    else if (te->clave > pa->datos->clave) pa->hd = abb_insertar_recursivo(a, pa->hd, te);
    
    else if (te->clave < pa->datos->clave) pa->hi = abb_insertar_recursivo(a, pa->hi, te);
...
void abb_insertar(ArbolBinarioBusqueda a, TipoElemento te) {
    if (abb_es_vacio(a)) {
        ...
        while (q != NULL) {
            ...
            if      (te->clave < q->datos->clave) q = q->hi;

            else if (te->clave > q->datos->clave) q = q->hd;
        ...
        if (q == NULL) {
            ...
            if (p->datos->clave > te->clave) p->hi = na;
            
            else p->hd = na;
        } 
        
        else printf(">> Clave duplicada: %d \n", te->clave); 
    }
...
NodoArbol abb_eliminar_recursivo(NodoArbol root, int claveABorrar) {
    ...
    if      (claveABorrar < root->datos->clave) root->hi = abb_eliminar_recursivo(root->hi, claveABorrar);

    else if (claveABorrar > root->datos->clave) root->hd = abb_eliminar_recursivo(root->hd, claveABorrar);
...
TipoElemento abb_buscar_recursivo(NodoArbol nodoArbol, int clave) {
    ...
    else if (clave < nodoArbol->datos->clave) return abb_buscar_recursivo(nodoArbol->hi, clave);

    else if (clave > nodoArbol->datos->clave) return abb_buscar_recursivo(nodoArbol->hd, clave);
