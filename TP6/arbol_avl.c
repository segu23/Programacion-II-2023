...
NodoArbol avl_insertar_recursivo(ArbolAVL a, TipoElemento te, NodoArbol pa) {
    ...
    if (balanceState == DESBALANCEADO_IZQUIERDA) {
        ...
        if (te->clave < pa->hi->datos->clave) pa = avl_rotar_derecha(pa);// Caso rotación derecha
    ...
    if (balanceState == DESBALANCEADO_DERECHA) {
        if (te->clave > pa->hd->datos->clave) pa = avl_rotar_izquierda(pa);// Caso rotación izquierda
